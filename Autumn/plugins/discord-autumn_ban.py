#!/usr/bin/python
# -------------------------------------------------------------------------
# discord-autumn_ban.py - Bans a user from the server.
# Author: Jason Drawdy (Buddha)
# Date: 4.10.20
# Version: 3.0.0 (Strawberry)
#
# Description:
# This module bans a specified user from the Discord server.
# -------------------------------------------------------------------------
class Plugin(object):
                    ...
                    try:
                        ...
                        for member in guild.members:
                            ...
                            if user == id:
                                ...
                                if not name.find('Autumn'):
                                    await self.message.channel.send("I can't ban myself from the server, silly.")
                                    me = True
                                else:
                                    await member.ban()
                                    banned = True
                        ...
                        if not me:
                            if not banned:
                                ...
                                await self.message.channel.send(member.mention + " has been banned from the server!")
                    except: pass
                    #print(user)
                else:
